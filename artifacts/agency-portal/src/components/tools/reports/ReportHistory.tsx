  ...
  return (
    <div className="rounded-xl border border-card-border bg-card p-4">
      ...
      <div className="space-y-2">
        {reports.map((report) => (
          <div key={report.id} className="rounded-lg border border-border p-3 flex items-center justify-between gap-3">
            <div>
            ...
            <button onClick={() => onRegenerate(report)} className="px-2.5 py-1.5 text-xs rounded-md bg-secondary text-secondary-foreground">
              Rigenera
            </button>
          </div>
        ))}
