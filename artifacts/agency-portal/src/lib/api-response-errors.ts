...
export function describeApiFailureForUser(
  ...
  if (s.startsWith("<!DOCTYPE") || s.toLowerCase().startsWith("<html")) {
    if (/publishable key|wrong host|invalid api key/i.test(s)) {
      return [
        "La richiesta non sta raggiungendo l'API di questo progetto (Node/Express su Render): la risposta HTML sembra provenire da un altro host o da un gateway di autenticazione.",
        "",
        "Cosa controllare: su Vercel servono i rewrite da /api verso l'URL del tuo servizio Render (vedi vercel.json nel repo). In locale, avvia l'api-server e verifica VITE_API_PROXY_TARGET / la proxy /api in vite.config verso quella porta.",
      ].join("\n");
    }
