...
function ClientLogo({ name, color, logoUrl }: { name: string; color?: string; logoUrl?: string | null }) {
  ...
  if (logoUrl) {
    return <img src={logoUrl} alt={safeName} className="w-full h-full object-contain p-2" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />;
  }
...
export default function Clients() {
  ...
  const handleCreate = async () => {
    ...
    const finalName =
      ...
      String(form.ragioneSociale ?? "").trim() ||
    ...
    try {
      const res = await fetch("/api/clients", {
        ...
        body: JSON.stringify({
          ...form,
          name: finalName,
          company: form.nomeCommerciale || finalName,
          tags: String(form.tags ?? "").split(",").map((t) => t.trim()).filter(Boolean),
        }),
      });
      const rawText = await res.text();
      const data = (() => {
        try { return rawText ? JSON.parse(rawText) : {}; } catch { return { raw: rawText }; }
      })();
      if (!res.ok) {
        const technical =
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 mb-6 shadow-sm">
            ...
            {activeTab === "generali" && (
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs text-muted-foreground">Nome azienda</label><input className="w-full mt-1 px-3 py-2 text-sm border border-input rounded-lg bg-background" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
                ...
                <div><label className="text-xs text-muted-foreground">Tag (separati da virgola)</label><input className="w-full mt-1 px-3 py-2 text-sm border border-input rounded-lg bg-background" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} /></div>
                <div><label className="text-xs text-muted-foreground">Colore brand</label><input type="color" className="h-9 w-20 border border-input rounded-lg bg-background mt-1" value={form.brandColor} onChange={(e) => setForm({ ...form, brandColor: e.target.value, color: e.target.value })} /></div>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={handleCreate} disabled={isSaving} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-60">{isSaving ? "Salvataggio..." : "Salva Cliente"}</button>
                ...
                <div className="group bg-card border border-card-border rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all cursor-pointer relative">
                  <div className="p-4 flex items-center gap-3">
                    <button
                      ...
                      style={{ backgroundColor: client.logoUrl ? "#f8f8f6" : client.brandColor ?? client.color }}
                    >
